...
public class LB_LightingBoxHelper : MonoBehaviour {
	...
	void Start()
	{
		...
		UpdateCamera (mainLightingProfile);
		UpdatePostEffects (mainLightingProfile);
		UpdateSettings (mainLightingProfile);
	}


	void UpdateCamera(LB_LightingProfile profile)
	{

		if(profile.camMode == CameraMode.All)
			cameraList = GameObject.FindObjectsOfType<Camera>();

		if(profile.camMode == CameraMode.Single)
		{
	...
	void UpdatePostEffects(LB_LightingProfile profile)
	{
		...
		helper.UpdateProfiles (cameraList, profile.postProcessingProfile);
		...
		helper.Update_Bloom(profile.Bloom_Enabled,profile.bIntensity,profile.bThreshould,profile.bColor,profile.dirtTexture,profile.dirtIntensity);


		// Depth of Field
		helper.Update_DOF(cameraList,profile.DOF_Enabled,profile.dofQuality,profile.dofBlur,profile.dofRange,profile.falloff,profile.visualize);
		helper.Update_AutoFocus (cameraList, profile.AutoFocus_Enabled, profile.DOF_Enabled,profile.afLayer, profile.afRange, profile.afBlur, profile.afSpeed, profile.afUpdate, profile.afRayLength);

		// AO
		if (profile.AO_Enabled)
			helper.Update_AO(cameraList,true,profile.aoType,profile.aoRadius,profile.aoIntensity,profile.ambientOnly,profile.aoColor, profile.aoQuality);
		else
			helper.Update_AO(cameraList,false,profile.aoType,profile.aoRadius,profile.aoIntensity,profile.ambientOnly,profile.aoColor, profile.aoQuality);


		// Color Grading
		helper.Update_ColorGrading(profile.colorMode,profile.exposureIntensity,profile.contrastValue,profile.temp,profile.eyeKeyValue,profile.saturation,profile.colorGamma,profile.colorLift,profile.gamma);

		////-----------------------------------------------------------------------------
		/// 
		// Screen Space Reflections
		helper.Update_SSR(cameraList, profile.SSR_Enabled,profile.ssrQuality,profile.ssrAtten,profile.ssrFade);

		helper.Update_StochasticSSR(cameraList, profile.ST_SSR_Enabled,profile.resolutionMode,profile.debugPass,profile.rayDistance,profile.screenFadeSize,profile.smoothnessRange);
	...
	void UpdateSettings(LB_LightingProfile profile)
	{
		...
		// Skybox
		helper.Update_SkyBox (profile.skyBox);
		...
		// Volumetric Light
		helper.Update_VolumetricLight(cameraList,profile.VL_Enabled,profile.vLight,profile.vLightLevel);

		// Sun Shaft
		helper.Update_SunShaft(cameraList,profile.SunShaft_Enabled, profile.shaftQuality,profile.shaftDistance,profile.shaftBlur,profile.shaftColor,sunLight.transform);

		// Global Fog
		helper.Update_GlobalFog(cameraList,profile.Fog_Enabled,profile.fogMode,profile.fogDistance,profile.fogHeight,profile.fogHeightIntensity,profile.fogColor,profile.fogIntensity);
	...
	public void Update_Bloom(bool enabled,float intensity,float threshold,Color color,Texture2D dirtTexture,float dirtIntensity)
	{
		if(enabled)
		{
			Bloom b;
	...
	public void Update_AutoFocus(Camera[] cams , bool enabled, bool dofEnabled,LayerMask afLayer,float afRange,float afBlur,float afSpeed, float afUpdate, float afRayLength)
	{
				...
				if (!enabled || !dofEnabled) {
					if (c.GetComponent<LB_AutoFocus> ()) {
						if(Application.isPlaying)
							Destroy(c.GetComponent<LB_AutoFocus> ());						
						else
	...
	public void Update_DOF(Camera[] cams ,bool enabled,DOFQuality quality,float blur,float range,float falloff, bool visualize)
	{
		foreach(Camera c in cams)
		{
			if (enabled) {
			...
			if (!enabled) {
				if (c.GetComponent<LightingBox.Effects.DepthOfField> ()) {
					if(Application.isPlaying)
						Destroy (c.GetComponent<LightingBox.Effects.DepthOfField> ());					
					else
	...
	public void Update_AA(Camera[] cams ,AAMode aaMode, bool  enabled)
	{
		foreach (Camera c in cams)
		{
			if (enabled) {
	...
	public void Update_ColorGrading(ColorMode colorMode,float exposureIntensity,float contrastValue,float temp,float eyeKeyValue
		,float saturation,Color colorGamma,Color colorLift,float gamma)
	{
		ColorGrading cg;
	...
	public void Update_SSR(Camera[] cams ,bool enabled,ScreenSpaceReflectionPreset preset,float atten,float fade)
	{
		...
		ssr.attenuation.overrideState = true;
		ssr.attenuation.value = atten;
	...
	public void Update_StochasticSSR(Camera[] cams ,bool enabled,ResolutionMode resolutionMode,SSRDebugPass debugPass
		,int rayDistance ,float screenFadeSize, float smoothnessRange )
	{
		foreach(Camera c in cams)
		{
			if (enabled) {
				if (!c.GetComponent<StochasticSSR> ()) {
					c.gameObject.AddComponent<StochasticSSR> ();
					StochasticSSR ssr = c.GetComponent<StochasticSSR> ();
			...
			if (!enabled) {
				if (c.GetComponent<StochasticSSR> ()) {
					if(Application.isPlaying)
						Destroy (c.GetComponent<StochasticSSR> ());					
					else
						DestroyImmediate (c.GetComponent<StochasticSSR> ());
				}
	...
	public void Update_RenderPath(bool enabled, Render_Path renderPath,Camera[] allCameras)
	{
		if (enabled) {
	...
	public void Update_SunShaft(Camera[] cams,bool enabled, UnityStandardAssets.ImageEffects.SunShafts.SunShaftsResolution shaftQuality,float shaftDistance,float shaftBlur, Color shaftColor, Transform sun)
	{
		if (!sun) 
		{
			...
			foreach (Camera c in cams) 
			{
				if (c.gameObject.GetComponent<UnityStandardAssets.ImageEffects.SunShafts> ()) {
					if(Application.isPlaying)
						Destroy (c.gameObject.GetComponent<UnityStandardAssets.ImageEffects.SunShafts> ());
					else
		...
		if (enabled) 
		{
			foreach (Camera c in cams)
			{
				...
				c.gameObject.GetComponent<UnityStandardAssets.ImageEffects.SunShafts> ().sunShaftsShader = Shader.Find
					("Hidden/SunShaftsComposite");
				c.gameObject.GetComponent<UnityStandardAssets.ImageEffects.SunShafts> ().simpleClearShader = Shader.Find
					("Hidden/SimpleClear");
	...
	public void Update_VolumetricLight(Camera[] cams, bool enabled, VolumetricLightType volumetricLight,VLightLevel vLightLevel)
	{
		if (enabled) {
		...
		if (!enabled) 
		{
			...
			foreach (Camera c in cams) 
			{
				if (c.gameObject.GetComponent<VolumetricLightRenderer> ()) {
					if(Application.isPlaying)
						Destroy (c.gameObject.GetComponent<VolumetricLightRenderer> ());					
					else
	...
	public void Update_GlobalFog(Camera[] cams, bool enabled, CustomFog fogMode,float fogDistance,float fogHeight,float fogHeightDensity,Color fogColor,float fogDensity)
	{
		if (enabled)
		{
	...
	private void UpdateFog(Camera[] cams,int fogType,float fogDistance,float fogHeight,float fogHeightDensity,Color fogColor,float fogDensity) // 0 Height , 1 Distance , 2 Global , 3 Off
	{
		...
		if (fogType == 3) {
			...
			foreach (Camera c in cams) 
			{
				if (c.gameObject.GetComponent<GlobalFog> ()) {
					if(Application.isPlaying)
						Destroy (c.gameObject.GetComponent<GlobalFog> ());					
					else
