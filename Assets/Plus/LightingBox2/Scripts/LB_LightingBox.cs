...
public class LB_LightingBox : EditorWindow
{
	...
	void OnGUI()
	{
		...
		if (winMode == WindowMode.Part1) {
			...
			var cameraStateRef = cameraState;

			if (GUILayout.Button ("Camera", stateButton, GUILayout.Width (300), GUILayout.Height (17f))) {
				cameraState = !cameraState;
			}

			if(cameraStateRef != cameraState)
			{
				if (LB_LightingProfile)
					LB_LightingProfile.cameraState = cameraState;
				if (LB_LightingProfile)
				...
				/*	if(GUILayout.Button("Clear Components"))
			{
				...
				if (camsRef != cameraList || camModeRef != camMode) 
				{
					UpdateCamera ();
					UpdatePostEffects ();
					UpdateSettings ();
					if (LB_LightingProfile)
					{
						LB_LightingProfile.camMode = camMode;
						EditorUtility.SetDirty (LB_LightingProfile);
					}
				}
			...
			var Sun_EnabledRef = Scene_Enabled;
			...
			if (renderPathRef != renderPath) {
					...
					helper.Update_RenderPath (Scene_Enabled,renderPath, cameraList);
			...
			if(vLightState)
			{
					...
					if (vLightRef != vLight || vLightLevelRef != vLightLevel || VL_EnabledRef != VL_Enabled) {
						...
						// Volumetric Light
						helper.Update_VolumetricLight (cameraList, VL_Enabled, vLight, vLightLevel);
						//----------------------------------------------------------------------
				if (LB_LightingProfile)
				{
			...
			if (SunShaft_EnabledRef != SunShaft_Enabled || shaftDistanceRef != shaftDistance
				  || shaftBlurRef != shaftBlur || shaftColorRef != shaftColor || shaftQualityRef != shaftQuality) {
					...
					// Sun Shaft
					helper.Update_SunShaft (cameraList, SunShaft_Enabled, shaftQuality, shaftDistance, shaftBlur, shaftColor, sunLight.transform);
			...
			{
					helper.Update_GlobalFog (cameraList, Fog_Enabled, vFog, fDistance, fHeight, fheightDensity, fColor, fogIntensity);
				if (LB_LightingProfile)
			...
			if(DOF_Enabled)
				...
				helper.Update_AutoFocus (cameraList, AutoFocus_Enabled, DOF_Enabled,afLayer, afRange, afBlur, afSpeed, afUpdate, afRayLength);
				...
				if (DOF_EnabledRef != DOF_Enabled || dofRangeRef != dofRange || dofBlurRef != dofBlur
				  ...
				  || visualizeRef != visualize) {
					...
					helper.Update_DOF (cameraList, DOF_Enabled, dofQuality, dofBlur, dofRange, falloff, visualize);
				helper.Update_AutoFocus (cameraList, AutoFocus_Enabled, DOF_Enabled,afLayer, afRange, afBlur, afSpeed, afUpdate, afRayLength);
			...
			if (dirtTextureRef != dirtTexture || dirtIntensityRef != dirtIntensity || Bloom_EnabledRef != Bloom_Enabled || bIntensityRef != bIntensity || bColorRef != bColor || bThreshouldRef != bThreshould || bIntensityRef != bIntensity) {
				...
				helper.Update_Bloom(Bloom_Enabled,bIntensity,bThreshould,bColor,dirtTexture,dirtIntensity);
			...
			if(colorState)
			{
				...
				if (exposureIntensityRef != exposureIntensity || contrastValueRef != contrastValue || tempRef != temp || eyeKeyValueRef != eyeKeyValue
				  || colorModeRef != colorMode || gammaRef != gamma || colorGammaRef != colorGamma || colorLiftRef != colorLift || saturationRef != saturation) {
					...
					helper.Update_ColorGrading (colorMode, exposureIntensity, contrastValue, temp, eyeKeyValue, saturation, colorGamma, colorLift, gamma);
			...
			if (aaModeRef != aaMode  || AA_EnabledRef != AA_Enabled) {
					...
					helper.Update_AA (cameraList, aaMode, AA_Enabled);
			...
			{
				helper.Update_SSR(cameraList, SSR_Enabled,ssrQuality,ssrAtten,ssrFade);
			...
			{
				helper.Update_StochasticSSR(cameraList, ST_SSR_Enabled,resolutionMode,debugPass,rayDistance,screenFadeSize,smoothnessRange);
		...
		void OnLoad()
		{
			...
			if (LB_LightingProfile) {
				camMode = LB_LightingProfile.camMode;
